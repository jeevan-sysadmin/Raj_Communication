...
const normalizeHandoverTypeMap = (value: unknown): Record<string, string> => {
  ...
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
  const allowed = new Set(["in_hand", "courier", "parcel_service"]);
  const result: Record<string, string> = {};
    ...
    try {
      ...
      const handoverTypeMap = Object.fromEntries(
        productIds
          ...
          .filter((productId) => ["in_hand", "courier", "parcel_service"].includes(String(incomingHandoverTypeMap[productId] || "")))
