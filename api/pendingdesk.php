...
function pendingdesk_is_assoc_array(array $value): bool {
    if ($value === []) return false;
    return array_keys($value) !== range(0, count($value) - 1);
}

function pendingdesk_parse_positioned_string_list($value): array {
    if ($value === null) return [];

    if (is_array($value)) {
        $raw = array_values($value);
    } elseif (is_string($value)) {
        $trimmed = trim($value);
        if ($trimmed === '') return [];
    ...
    return array_map(static fn($entry) => trim((string)$entry), $raw);
}

function pendingdesk_parse_keyed_string_map($value): array {
    $parsed = pendingdesk_parse_record($value);
    if (empty($parsed) || !pendingdesk_is_assoc_array($parsed)) return [];

    $normalized = [];
    foreach ($parsed as $key => $entry) {
        $normalized[trim((string)$key)] = trim((string)$entry);
    }
    return $normalized;
}
...
try {
    ...
    foreach ($orders as $order) {
        ...
        $productNames = pendingdesk_parse_positioned_string_list($order['product_names'] ?? null);
        $productModels = pendingdesk_parse_positioned_string_list($order['product_models'] ?? null);
        $productSerials = pendingdesk_parse_positioned_string_list($order['product_serial_numbers'] ?? null);
        $productNameMapById = pendingdesk_parse_keyed_string_map($order['product_names'] ?? null);
        $productModelMapById = pendingdesk_parse_keyed_string_map($order['product_models'] ?? null);
        $productSerialMapById = pendingdesk_parse_keyed_string_map($order['product_serial_numbers'] ?? null);
        ...
        foreach ($allProductIds as $index => $productId) {
            $metaByProductId[(int)$productId] = [
                'product_name' => $productNameMapById[(string)$productId] ?? ($productNames[$index] ?? ''),
        ...
        foreach ($matchedEntries as $companyEntry) {
            ...
            $companyScopedNameList = [];
            if ($companyEntry['id'] !== '' && isset($companyProductNameMap[$companyEntry['id']])) {
                $mappedNames = $companyProductNameMap[$companyEntry['id']]['product_names'] ?? [];
                $companyScopedNameList = pendingdesk_parse_positioned_string_list($mappedNames);
            }

            foreach ($scopedProductIds as $scopedIndex => $productId) {
                ...
                $resolvedProductName = trim((string)($metaByProductId[(int)$productId]['product_name'] ?? ''));
                if ($resolvedProductName === '' && isset($companyScopedNameList[$scopedIndex])) {
                    $resolvedProductName = trim((string)$companyScopedNameList[$scopedIndex]);
                }
    ...
    foreach ($rows as &$row) {
        ...
        if ($lookup) {
          ...
          if (trim((string)$lookup['serial']) !== '') $row['serial'] = $lookup['serial'];
          if (trim((string)$lookup['model']) !== '') $row['model'] = $lookup['model'];
        }
